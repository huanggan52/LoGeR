...
class Pi3Trainer(BaseTrainer):
    ...
    def build_optimizer(self, cfg_optimizer, model):
        def param_group_fn(model_):
            ttt_swa_prefixes = ('ttt_layers.', 'ttt_gate_projs.', 'swa_layers.', 'swa_gate_projs.')

            encoder_params = [(n, p) for n, p in model_.named_parameters()
                              if n.startswith('encoder.')]
            ttt_swa_params = [(n, p) for n, p in model_.named_parameters()
                              if any(n.startswith(pfx) for pfx in ttt_swa_prefixes)]
            base_params    = [(n, p) for n, p in model_.named_parameters()
                              if not n.startswith('encoder.')
                              and not any(n.startswith(pfx) for pfx in ttt_swa_prefixes)]

            print(f'Trainable encoder params:  {sum(p.numel() for _, p in encoder_params if p.requires_grad):,}')
            print(f'Trainable TTT/SWA params:  {sum(p.numel() for _, p in ttt_swa_params if p.requires_grad):,}')
            print(f'Trainable base params:     {sum(p.numel() for _, p in base_params if p.requires_grad):,}')
            ...
            res.extend(handle_weight_decay(ttt_swa_params, cfg_optimizer.weight_decay, cfg_optimizer.ttt_swa_lr))
            res.extend(handle_weight_decay(base_params,    cfg_optimizer.weight_decay, cfg_optimizer.lr))
            return res
