...
class Pi3(nn.Module):
    def __init__(
        ...
        # Freeze some parameters if specified
        self.train_conf = bool(train_conf)
        if train_conf:
        ...
        if freeze_head:
            print('Freezing the prediction heads for the local pointmap, global camera pose, and confidence map.')
            freeze_all_params([self.point_decoder, self.point_head, self.camera_decoder, self.camera_head, self.conf_decoder, self.conf_head])
