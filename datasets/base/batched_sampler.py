...
class FixedLengthBatchSampler(Sampler):
    """
    Like DynamicBatchSampler but uses a fixed sequence_length instead of
    sampling from image_num_range.  Every batch has the same frame count.

    Yields batches of (sample_idx, resolution_idx, sequence_length) tuples,
    compatible with BaseDataset.__getitem__ which unpacks a 3-tuple idx as
    (idx, ar_idx, frame_num).
    """
    ...
    def __init__(
        ...
        self.max_img_per_gpu = max_img_per_gpu
        self.rank = rank
        self.batch_size = max(1, max_img_per_gpu // sequence_length)
    ...
    def __iter__(self):
        ...
        while True:
            resolution_idx = self.rng_rank.choice(self.resolution_num)
            self.sampler.update_parameters(
                resolution_idx=resolution_idx,
                image_num=self.sequence_length,
            )

            current_batch = []
            for _ in range(self.batch_size):
                try:
                    item = next(sampler_iterator)  # (idx, resolution_idx, image_num)
                    current_batch.append(item)
                except StopIteration:
                    break
